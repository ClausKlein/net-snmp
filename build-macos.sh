...
if [ -x ${SRCDIR}/configure ]; then
  ...
  ${SRCDIR}/configure --cache-file="config.cache" \
    --prefix=/opt/local \
    --with-cflags="-I/opt/local/include" \
...
#
sudo killall snmpd
sudo ./agent/snmpd -Lf ./snmpd.log -p ${PWD}/snmpd.pid -C -Dread_config,mib,register_mib  \
...
sudo ./apps/snmpstatus -m ALL -M ../mibs -r0 -v3 -u clausklein -l noAuthNoPriv --defSecurityModel=usm -Dread,mib,ssh 'tcp6:[::1]:161'
